...
Feature: Converting a conversation to a task
  ...
  Scenario: Converting a simple conversation on the overview page and specifying additional task attributes
    ...
    And I select the year "2024" with the date picker
    ...
    # Comments are not being persisted (but changes does) when body is blank.
    # This can be commented-out once that's fixed.
  ...
  Scenario: I can set the urgent flag for the task being created
    ...
    Then I should see "Urgent" within ".fields-due-on"
    When I press "Convert"
    And I wait for 3 seconds
    Then I should see "Give git course" in the task thread title
