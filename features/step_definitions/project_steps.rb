Given /^there is a project called "([^\"]*)"$/ do |name|
  Project.find_by_name(name) || FactoryBot.create(:project, name: name)
end

Given /^"([^\"]*)" sent an invitation to "([^\"]*)" for the project "([^\"]*)"$/ do |login, email, project_name|
  user = User.find_by_login(login)
  project = Project.find_by_name(project_name)
  FactoryBot.create(:invitation, user: user, email: email, project: project)
end
...
Given(/^a project with users? (.+)$/) do |users|
  @current_project = FactoryBot.create(:project)
...
Given /^there is a project with a conversation$/ do
  @current_project = @current_user.projects.first || FactoryBot.create(:project)
