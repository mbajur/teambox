...
When /^(?:|I )check the checkbox "([^\"]*)"(?: within "([^\"]*)")?$/ do |locator, selector|
  with_scope(selector) do
    ...
    find(:xpath, ".//input[./@type = 'checkbox'][((./@id = '#{locator}' or ./@name = '#{locator}') or ./@id = //label[contains(./text(), '#{locator}')]/@for)] | .//label[contains(normalize-space(.), '#{locator}')]//.//input[./@type = 'checkbox']").set(true)
  end
end
