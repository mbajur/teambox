...
module Emailer::Incoming
  ...
  def process_incoming(email)
    ...
    configured_domain = Rails.configuration.teambox.smtp_settings[:domain]
  ...
  def post_to(target)
    Rails.logger.info "Posting to #{target.class} #{target.id} '#{@subject}'"
  ...
  def create_task
    ...
    task_list_name = Rails.configuration.teambox.incoming_email_settings[:inbox_task_list] || "Inbox"
