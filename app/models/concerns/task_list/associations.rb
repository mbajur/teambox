module TaskList::Associations
  ...
  included do
    ...
    has_many :archived_tasks, -> { order("tasks.position").where("status >= ?", 3).includes(:project, :task_list, :assigned) }, class_name: "Task"
    has_many :unarchived_tasks, -> { order("tasks.position").where("status < ?", 3).includes(:project, :task_list, :assigned) }, class_name: "Task"
  end
end
