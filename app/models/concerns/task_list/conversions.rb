module TaskList::Conversions
  ...
  def to_xml(options = {})
    ...
    xml.task_list id: id do
      ...
      xml.tag! "start-on",        start_on.to_fs(:db) if start_on
      xml.tag! "finish-on",       finish_on.to_fs(:db) if finish_on
  ...
  def to_api_hash(options = {})
    ...
    base[:start_on] = start_on.to_fs(:db) if start_on
    base[:finish_on] = finish_on.to_fs(:db) if finish_on
