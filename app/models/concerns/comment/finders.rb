module Comment::Finders
  ...
  class_methods do
    ...
    def find_by_year(year = nil)
      year ||= Time.new.year
      find(:all, conditions: [ "YEAR(created_at) = ?", year ], order: "created_at DESC")
    end

    def find_by_year_count(year = nil)
      year ||= Time.new.year
      count(:all, conditions: [ "YEAR(created_at) = ?", year ])
    end

    def find_by_month(month_number = nil, year_number = nil)
    ...
    def find_by_week(week_number = nil)
