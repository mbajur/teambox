module User::Avatar
  ...
  included do
    has_attached_file :avatar, url: "/avatars/:id/:style.png",
                                    path: (Rails.configuration.teambox.amazon_s3 ? "avatars/:id/:style.png" : ":rails_root/public/avatars/:id/:style.png"),
                                    styles: AvatarSizes.each_with_object({}) { |(name, size), all|
                                        all[name] = [ "%dx%d%s" % [ size[0], size[1], size[0] < size[1] ? ">" : "#" ], :png ]
                                      }

    attr_accessor :avatar, :avatar_destroy
