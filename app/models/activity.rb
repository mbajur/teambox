class Activity < ApplicationRecord
  ...
  scope :limit_per_page, -> { limit(Rails.configuration.teambox.activities_per_page) }
  ...
  def self.in_projects(projects)
    where("activities.project_id IN (?)", Array(projects).collect(&:id))
  end
  ...
  def ping_parent_activity
    if target.is_a? Comment and parent = Activity.where("target_type = ? AND target_id = ?", comment_target_type, comment_target_id).last
