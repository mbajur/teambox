class TaskListsController < ApplicationController
  ...
  def create
    authorize! :make_task_lists, @current_project
    @on_index = true
    if task_list_params[:template].present? and task_list_params[:name].blank?
      if template = @current_project.organization.task_list_templates.find(task_list_params[:template])
        @task_list = template.create_task_list(@current_project, current_user)
      end
    else
      @task_list = @current_project.create_task_list(current_user, task_list_params)
    end
  ...
  def update
    authorize! :update, @task_list
    calc_onindex
    @saved = @task_list.update(task_list_params)
  ...
  private
    def task_list_params
      params.require(:task_list).permit(:name, :description, :start_on, :finish_on, :template)
    end
    ...
    def load_task_lists
      if @current_project
        @task_lists = @current_project.task_lists.includes(:project)
      else
