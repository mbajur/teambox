class UsersController < ApplicationController
  skip_before_action :require_authentication, only: [ :new, :create, :confirm_email ]

  before_action :find_user, only: [ :show, :confirm_email ]
  before_action :load_invitation, only: [ :new, :create ]
  # skip_before_action :confirmed_user?, only: [ :new, :create, :confirm_email, :forgot_password, :reset_password, :login_from_reset_password, :unconfirmed_email ]
  skip_before_action :load_project
  before_action :set_page_title
  before_action :can_users_signup?, only: [ :new, :create ]
  ...
  def update
    ...
    respond_to do |wants|
      wants.html {
        if success
          back = polymorphic_url([ :account, @sub_action.to_sym ])
  ...
  private
    ...
    def can_users_signup?
      unless @invitation || signups_enabled?
        ...
        redirect_to Rails.configuration.teambox.community ? login_path : root_path
      end
    end
