...
module ApplicationHelper
  ...
  def show_flash
    flash.each do |type, message|
      unless message.blank?
        concat tag.p(message.html_safe, class: "flash flash-#{type}")
      end
    end
  ...
  def help_link
    if url = Rails.configuration.teambox.help_url
  ...
  def configure_this_organization
    if Rails.configuration.teambox.community && @community_role == :admin && @community_organization.description.blank? && params[:organization].nil?
  ...
  def tracking_code
    if Rails.configuration.teambox.tracking_enabled and Rails.env.production?
