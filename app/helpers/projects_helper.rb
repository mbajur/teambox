module ProjectsHelper
  ...
  def new_project_link
    if !Rails.configuration.teambox.community || (@community_organization && !@community_role.nil?)
  ...
  def instructions_for_email(project)
    ...
    when "show_projects" # project@app.teambox.com
        email_help = t("shared.instructions.send_email_help_project_html", email: "#{project.permalink}@#{Rails.configuration.teambox.smtp_settings[:domain]}")
    when "show_tasks" # project+task+12@app.teambox.com
        email_help = t("shared.instructions.send_email_help_task_html", email: "#{project.permalink}+task+#{@task.id}@#{Rails.configuration.teambox.smtp_settings[:domain]}")
    when "index_conversations" # project+conversation@app.teambox.com
        email_help = t("shared.instructions.send_email_help_conversations_html", email: "#{project.permalink}+conversation@#{Rails.configuration.teambox.smtp_settings[:domain]}")
    ...
    when "show_conversations" # project+conversation+5@app.teambox.com
        email_help = t("shared.instructions.send_email_help_conversation_html", email: "#{project.permalink}+conversation+#{@conversation.id}@#{Rails.configuration.teambox.smtp_settings[:domain]}")
    end
