...
describe CommentsController, type: :controller do
  ...
  describe "#create" do
    it "should set the current user as the author" do
      @jordi = FactoryBot.create(:confirmed_user, login: 'jordi')
      conversation = FactoryBot.create(:conversation, user: @jordi, project: @project)
  ...
  describe "#destroy rollback" do
    ...
    it "reverts the status back to the previous status when destroyed as the last comment with do_rollback" do
      ...
      delete :destroy, params: { project_id: @project.permalink, task_id: @task.id, id: @task.comments.last.id }
    ...
    it "maintains the status if any other comments are destroyed" do
      login_as @project.user
      delete :destroy, params: { project_id: @project.permalink, task_id: @task.id, id: @task.comments[1].id }
