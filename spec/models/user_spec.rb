...
describe User, type: :model do
  ...
  it { should validate_length_of(:login).is_at_least(3).is_at_most(40) }
  xit { should validate_uniqueness_of(:login) }
  it { should validate_confirmation_of(:password) }

  # TODO: Validates format of login, name and email

  it { should validate_length_of(:first_name).is_at_least(1).is_at_most(20) }
  it { should validate_length_of(:last_name).is_at_least(1).is_at_most(20) }

  xit { should validate_presence_of(:email) }
  xit { should validate_length_of(:email).is_at_least(6).is_at_most(100) }
  ...
  describe "authentication" do
    ...
    xit "should return the user object for a valid login using his email" do
      User.authenticate(@email, @password).should == @user
      User.authenticate(@email.upcase, @password).should == @user
    end

    xit "should return nil for incorrect login attempts" do
  ...
  describe "activation" do
    ...
    it "should generate a different and valid login token each time" do
      ...
      @user.is_login_token_valid?(@user.login_token).should be true
    end
    ...
    it "should expire tokens when asked for" do
      ...
      @user.is_login_token_valid?(login_token).should be false
    end

    describe "activation email" do
      xit "should send an activation email" do
  ...
  describe "factories" do
    it "should generate Mislav for use in Cucumber stories" do
      mislav = FactoryBot.create(:mislav)
      mislav.valid?.should be true
  ...
  describe "when fetching the user in a project" do
    ...
    it "should return nil if the user is not part of the project" do
      FactoryBot.create(:user).in_project(@project).should be_nil
    end
  end
  ...
  describe "deletion" do
    ...
    it "has a method to rename the user as the original name" do
      @user = User.unscoped.immortal.first
      @user.recover!
  ...
  describe "#assigned_tasks_count" do
    ...
    it "should return assigned tasks count" do
      @user.assigned_tasks_count.should == nil
