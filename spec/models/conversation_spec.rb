...
describe Conversation, type: :model do
  it "creates with first comment" do
    conversation = FactoryBot.build(:simple_conversation, body: nil)
  ...
  it "fails with blank comment" do
    ...
    lambda {
      conversation.save.should be false
      conversation.errors['comments'].should == [ "The conversation cannot start with an empty comment." ]
    }.should_not change(described_class, :count)
  end

  it "fails with blank name if not simple" do
    conversation = FactoryBot.build(:conversation, name: "", simple: false)
    conversation.save.should be false
    pp conversation.errors['name'].should == [ "Please give this conversation a title." ]
  end

  it "allows blank name if simple" do
    expect { FactoryBot.create(:conversation, name: "", simple: true) }
      .to change(Conversation, :count)
      .by(1)
  end

  it "destroy itself when the last comment is destroy if simple" do
    conversation = FactoryBot.build(:conversation, simple: true)
    conversation.user.save
    conversation.save!
    @comment = conversation.comments.first.destroy
    Conversation.find_by(id: @comment.target.id).should be_nil
  end
  ...
  it "change to normal conversation if title is added to simple conversation" do
    ...
    conversation.simple.should be true
  ...
  describe "is convertable to a task" do
    it "if is not simple" do
      conversation = FactoryBot.create(:conversation, simple: false)
    ...
    it "and when converted to a task, the task should retain same created_at timestamp" do
      conversation = FactoryBot.create(:conversation, simple: false, created_at: 2.hours.ago)
    ...
    it "and when converted to a task, the task's updated_at timestamp should be 'touched'" do
      conversation = FactoryBot.create(:conversation, simple: false, created_at: 2.hours.ago, updated_at: 1.hour.ago)
    ...
    it "and when converted to a task, the comments should be transferred to the task" do
      conversation = FactoryBot.build(:conversation, simple: false)
      conversation.user.save
      conversation.comments_attributes = { "0" => { body: "Just sayin' hi" } }
      conversation.save!
      ...
      task.should_not be_nil
      task.comments.all? { |comment| conversation_comments.include?(comment.id) }.should be true
    end
    ...
    it "and when converted to a task, any new comments should also be transferred to the task" do
      ...
      task.should_not be_nil
      task.comments.any? { |comment| comment.body == "Just sayin' hi" }.should be true
    end
    ...
    xit "and when converted to a task with due date, it should update the comment counter cache" do
      conversation = FactoryBot.create(:conversation, simple: false)
      conversation.comments_attributes = { "0" => { body: "Just sayin' hi" } }
      ...
      task.should_not be_nil
      task.comments.to_a.size.should == 3
    ...
    it "and when converted to a task, any new comment which fails validation should not be transferred to the task" do
      ...
      task = conversation.convert_to_task!
      task.should_not be_nil
      task.comments.any? { |comment| comment.body == "Just sayin' hi" }.should be false
    end

    xit "and when converted to a task, the activities should be transferred to the task" do
      conversation = FactoryBot.build(:conversation, simple: false)
      conversation.user.save
      conversation.comments_attributes = { "0" => { body: "Just sayin' hi" } }
      conversation.save!
    ...
    it "and when converted to a task, the task should have a due date if supplied" do
      ...
      task.due_on.to_date.should == due_on.to_date
    end

    it "but should not be convertable to a task if simple and doesn't supply name" do
      conversation = FactoryBot.create(:conversation, name: nil, simple: true)
    ...
    it "if simple and a name is supplied" do
      conversation = FactoryBot.create(:conversation, simple: true)
      conversation.should be_valid
    ...
    it "all related comments and activities should be private" do
      conversation = FactoryBot.create(:conversation, is_private: true, simple: false)
      conversation.comments_attributes = { "0" => { body: "Just sayin' hi" } }
      conversation.save!
      task = conversation.convert_to_task!
      task.should_not be_nil
      task.is_private.should == true
      task.comments.any? { |c|c.is_private }.should == true
  ...
  describe "private conversations" do
    it "should mark all related activities as private when created as private" do
      conversation = FactoryBot.create(:conversation, is_private: true)
      activities_for_thread(conversation) { |activity| activity.is_private.should == true }
    end

    # @todo move out from Paperclip first
    xit "should update the private status of related activities and comments each time its updated" do
      conversation = FactoryBot.create(:conversation, is_private: true)
      ...
      conversation.update(is_private: false)
      activities_for_thread(conversation) { |activity| pp activity; activity.is_private.should == false }
    ...
    it "private_ids can only be changed when is_private is set" do
      watcher = FactoryBot.create(:user)
      project = FactoryBot.create(:project)
      conversation = FactoryBot.create(:conversation, is_private: true, project: project, user: project.user)
    ...
    xit "is_private cannot be mass assigned" do
      project = FactoryBot.create(:project)
